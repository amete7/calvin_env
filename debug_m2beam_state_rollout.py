...
from dataset.dataset_calvin import CustomDataset_Cont, CustomDataset_ABCD_lang_no_pad
...
def get_beam_search_sequences(model, attach_emb, device):
    # dummy_index1 = (torch.ones((1))*cfg.prior.pad_token[0]).long().to(device)
    # dummy_index2 = (torch.ones((1))*cfg.prior.pad_token[1]).long().to(device)
    # dummy_index3 = (torch.ones((1))*cfg.prior.pad_token[2]).long().to(device)
    inputs = torch.tensor((cfg.prior.pad_token[0], cfg.prior.pad_token[1], cfg.prior.pad_token[2])).long().to(device)
    outputs = beam_search_decode(model, inputs, attach_emb, 32, 5)
    print(outputs)
    return outputs
...
def main(cfg):
    ...
    if save_video:
        if cfg.use_prior:
            output_video_path = f'{cfg.action_horizon}_{data_type}_{cfg.prior_type}_roll_{traj_idx}_top_k_t.mp4'
        else:
    ...
    custom_dataset = CustomDataset_ABCD_lang_no_pad(processed_data_path,pred_horizon=cfg.action_horizon, pad_length=cfg.action_horizon//2)
    ...
    obs = torch.cat((scene_state,robot_state),dim=-1).float().unsqueeze(1).to(device)
    ...
    for i in range(cfg.num_steps):
        obs_to_use = torch.cat(list(obs_ctx), dim=1)
        # print(obs_to_use.shape,'obs shape')
        # print(lang_emb.shape,'lang_emb shape')
        with torch.no_grad():
            # index = get_index(gpt_prior_model, (lang_emb, obs_to_use), list(indices), device)
            indices = get_beam_search_sequences(gpt_prior_model, (lang_emb, obs_to_use), device)
