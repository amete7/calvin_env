...
def get_indices(init_obs,lang_prompt,task_oracle,prior_dict):
    ...
    scene_distances = np.linalg.norm(data_obs - init_obs[:,6:24], axis=1)
    closest_indices = np.argsort(scene_distances)[:32]
    # print(closest_indices, 'closest_indices')
    print(scene_distances[closest_indices][0], 'scene distances in obs')
    closest_data_obs = prior_dict['init_obs_emb'][data_index][closest_indices]
    # print(closest_data_obs.shape, 'closest_data_obs shape')
    robot_distances = np.linalg.norm(closest_data_obs[:,24:30] - init_obs[:,24:30], axis=1)
    closest_index = np.argmin(robot_distances)
    print(robot_distances[closest_index],'robot distance in obs')
    actual_closest_index = closest_indices[closest_index]
    print(actual_closest_index, 'actual_closest_index')
    closest_init = prior_dict['init_obs_emb'][data_index[actual_closest_index]]
    closest_code = prior_dict['codes'][data_index[actual_closest_index]]
    print(data_index[actual_closest_index], 'index from data')
    # print(closest_code, 'closest_code')
    # print(closest_init, 'closest_init')
    return closest_code, closest_init[:24]
...
def main(cfg):
    ...
    custom_dataset = CustomDataset_Cont(processed_data_path,pred_horizon=cfg.model.skill_block_size, pad_length=cfg.model.skill_block_size//2)
