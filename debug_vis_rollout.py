...
resnet.eval()
resnet = torch.nn.Sequential(*list(resnet.children())[:-1]) # before linear
...
def get_resnet18_features(image_tensor):
    ...
    with torch.no_grad():
        return resnet(image_tensor.to(device)).squeeze(2).squeeze(2)
...
def main(cfg):
    ...
    if save_video:
        if cfg.use_prior:
            output_video_path = f'{data_type}_{cfg.prior_type}_roll_{idx//57}.mp4'
        else:
            output_video_path = f'{data_type}_endec_roll_{idx//57}.mp4'
        frame_size = (400,400)
        fps = 15
    ...
    gpt_config = GPTConfig(vocab_size=cfg.prior.vocab_size, block_size=cfg.prior.block_size, output_dim=cfg.prior.output_dim, discrete_input=True, obs_size=cfg.prior.obs_size)
    ...
    action_horizon = 32
    for i in range(32//action_horizon):
        data = custom_dataset[idx + action_horizon*i +1]
        ...
        print(indices, 'indices from encoder')
        init_emb = obs[:, 0, ...]
        lang_emb = get_language_features(lang_prompt)
        if cfg.use_prior:
            with torch.no_grad():
                if cfg.prior_type == 'gpt':
                    indices = get_indices(gpt_prior_model, (lang_emb,init_emb), cfg.prior.attach_pos, device)
                else:
