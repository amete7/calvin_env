...
class SkillAutoEncoder(nn.Module):
    def __init__(self, conf):
        ...
        if self.vq_type == 'vq-vae':
            ...
            self.prediction_head = nn.Linear(self.input_emb * self.skill_block_size, len(self.fsq_level))
        ...
        if self.decoder_type == 'mlp':
            ...
            self.relu = nn.ReLU()
            self.hidden_to_action = nn.Linear(self.gru_hidden_dim, self.skill_block_size * self.action_dim)
        else:
    ...
    def decode_eval(self, x, init=None):
        ...
        if self.decoder_type == 'mlp':
            ...
            out = self.hidden_to_action(out)
            # print(out.shape, 'out_shape')
        out = out.view(1, 32, self.action_dim)
    ...
    def forward(self, obs, act):
        ...
        out, vq_loss, pp = self.decode(out, init=obs[:, 0, ...])
        return out, vq_loss, pp
...
if __name__ == '__main__':
    ...
    print(out.shape)
    out, vq_loss, pp = net.decode(out, init=obs[:, 0, ...])
