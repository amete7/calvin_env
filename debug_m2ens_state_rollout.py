...
def main(cfg):
    ...
    if save_video:
        if cfg.use_prior:
            output_video_path = f'{cfg.action_horizon}_{data_type}_{cfg.prior_type}_roll_{traj_idx}_mpc8_beam_5.mp4'
        else:
    ...
    for i in range(cfg.num_steps*4):
        with torch.no_grad():
        ...
        # print(action,'action')
        for timestep in tqdm(range(cfg.action_horizon//4)):
