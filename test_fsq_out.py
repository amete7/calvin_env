...
from model_sv import SkillAutoEncoder
from torch.utils.data import Dataset

class CustomDataset_Shared(Dataset):
    def __init__(self, dataset_dict_path):
        data_dict = np.load(dataset_dict_path,allow_pickle=True)
        self.front_embs = torch.from_numpy(data_dict['front_embs'])
        self.actions = torch.from_numpy(data_dict['actions'])

    def __len__(self):
        return len(self.front_embs)

    def __getitem__(self, idx):
        obs_sequence = self.front_embs[idx]
        action_sequence = self.actions[idx]

        return {'obs': obs_sequence, 'action': action_sequence}

dataset_folder_path = "/satassdscratch/scml-shared/calvin_data/task_D_D/clip_data.npz"
custom_dataset = CustomDataset_Shared(dataset_folder_path)
...
def main(cfg):
    ...
    skill_block_size = 32
    save_video = True
    ckpt_path = "/satassdscratch/scml-shared/calvin_data/task_D_D/ckpt_sv450.bin"
    if save_video:
    ...
    observation = custom_dataset[4]
    obs = observation['obs'].unsqueeze(0).to(device)
    action = observation['action'].unsqueeze(0).to(device)
    ...
    out = model.encode(obs,action)
    # print(out.shape,'out_shape')
    out = model.prediction_head(out)
    _,indices = model.vq(out)
