...
def main(cfg):
    ...
    skill_block_size = 32
    save_video = True
    ckpt_path = "/satassdscratch/scml-shared/calvin_data/task_D_D/ckpt_v2450.bin"
    if save_video:
    ...
    # indices = torch.randint(0, 1000, (1,)).to(device)
    indices = torch.tensor([500]).to(device)
    z = model.vq.indices_to_codes(indices).to(device)
